...
class PGDriveSceneCull:
    ...
    @classmethod
    def cull_distant_blocks(cls, pgdrive_engine, blocks: list, poses: List[tuple], max_distance=None):
        ...
        for block in blocks:
            if not cls.all_out_of_bounding_box(block.bounding_box, poses, cls.LOD_MAP_VIS_DIST):
                if not block.node_path.hasParent():
                    block.node_path.reparentTo(pgdrive_engine.worldNP)
            else:
            ...
            if not cls.all_out_of_bounding_box(block.bounding_box, poses, max_distance or cls.LOD_MAP_PHYSICS_DIST):
                block.dynamic_nodes.attach_to_physics_world(pgdrive_engine.physics_world.dynamic_world)
            else:
                block.dynamic_nodes.detach_from_physics_world(pgdrive_engine.physics_world.dynamic_world)
        ...
        for obj in elements:
            ...
            if not cls.all_distance_greater_than(vis_distance, poses, v_p):
                if not obj.node_path.hasParent():
                    obj.node_path.reparentTo(pgdrive_engine.pbr_worldNP)
            else:
                if obj.node_path.hasParent():
                    obj.node_path.detachNode()

            if not cls.all_distance_greater_than(physics_distance, poses, v_p):
                obj.dynamic_nodes.attach_to_physics_world(pgdrive_engine.physics_world.dynamic_world)
            else:
                obj.dynamic_nodes.detach_from_physics_world(pgdrive_engine.physics_world.dynamic_world)
