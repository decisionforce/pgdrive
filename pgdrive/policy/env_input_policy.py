...
class EnvInputPolicy(BasePolicy):
    def __init__(self):
        ...
        self.steering_unit = 2.0 / (
                    self.engine.global_config["discrete_steering_dim"] - 1)  # for discrete actions space
        self.throttle_unit = 2.0 / (
                    self.engine.global_config["discrete_throttle_dim"] - 1)  # for discrete actions space
