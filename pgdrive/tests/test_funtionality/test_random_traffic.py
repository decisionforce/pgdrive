...
def test_random_traffic():
    ...
    try:
        ...
        for i in range(20):
            obs = env.reset()
            assert env.scene_manager._traffic_manager.random_traffic
            assert env.scene_manager._traffic_manager.random_seed is None
            new_pos = [v.position for v in env.scene_manager._traffic_manager.vehicles]
