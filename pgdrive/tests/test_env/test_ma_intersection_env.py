...
from pgdrive.envs.marl_envs.ma_intersection import MultiAgentIntersectPGDrive
...
def test_ma_intersection_env():
    env = MultiAgentIntersectPGDrive({"num_agents": 1, "vehicle_config": {"lidar": {"num_others": 8}}})
    ...
    env = MultiAgentIntersectPGDrive({"num_agents": 1, "vehicle_config": {"lidar": {"num_others": 0}}})
    try:
        obs = env.reset()
        assert env.observation_space.contains(obs)
        for step in range(100):
    ...
    env = MultiAgentIntersectPGDrive({"num_agents": 4, "vehicle_config": {"lidar": {"num_others": 8}}})
    try:
        obs = env.reset()
        assert env.observation_space.contains(obs)
        for step in range(100):
    ...
    env = MultiAgentIntersectPGDrive({"num_agents": 4, "vehicle_config": {"lidar": {"num_others": 0}}})
    try:
        obs = env.reset()
        assert env.observation_space.contains(obs)
        for step in range(100):
    ...
    env = MultiAgentIntersectPGDrive({"num_agents": 8, "vehicle_config": {"lidar": {"num_others": 0}}})
    try:
        obs = env.reset()
        assert env.observation_space.contains(obs)
        for step in range(100):
...
def test_ma_intersection_reset():
    env = MultiAgentIntersectPGDrive({"horizon": 50, "num_agents": 4})
    try:
        obs = env.reset()
        assert env.observation_space.contains(obs)
    ...
    env = MultiAgentIntersectPGDrive({"horizon": 100, "num_agents": 32, "success_reward": 777})
    try:
        success_count = 0
        agent_count = 0
        obs = env.reset()
...
def test_ma_intersection_close_born():
    ...
    MultiAgentIntersectPGDrive._DEBUG_RANDOM_SEED = 1
    env = MultiAgentIntersectPGDrive({"horizon": 50, "num_agents": 16, "map_config": {"exit_length": 20}})
    env.seed(100)
    try:
        ...
        env.close()
        MultiAgentIntersectPGDrive._DEBUG_RANDOM_SEED = None


def test_ma_intersection_reward_done_alignment():
    # out of road
    env = MultiAgentIntersectPGDrive({"horizon": 200, "num_agents": 4, "out_of_road_penalty": 777, "crash_done": False})
    try:
        obs = env.reset()
        assert env.observation_space.contains(obs)
        for action in [-1, 1]:
...
if __name__ == '__main__':
    ...
    test_ma_intersection_close_born()
