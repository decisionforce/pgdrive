...
class BaseVehicle(DynamicElement):
    ...
    def update_map_info(self, map):
        ...
        possible_lanes = ray_localization(
            np.array(self.heading.tolist()), np.array(self.spawn_place), self.pg_world, return_all_result=True
        )
    ...
    def general_done(self):
        return (
            self.overspeed or self.crash_sidewalk or self.crash_vehicle or self.crash_object or self.crash_building
            or self.on_yellow_continuous_line or self.on_white_continuous_line or self.out_of_route
        )
