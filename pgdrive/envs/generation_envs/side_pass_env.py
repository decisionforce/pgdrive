...
class SidePassEnv(PGDriveEnv):
    ...
    def reset(self, episode_data: dict = None):
        ...
        self.breakdown_vehicle = self.scene_manager._traffic_manager.spawn_one_vehicle(
            self.scene_manager._traffic_manager.random_vehicle_type(), lane, 30, False
        )
        ...
        breakdown_vehicle = self.scene_manager._traffic_manager.spawn_one_vehicle(LVehicle, lane_, 30, False)
        breakdown_vehicle.attach_to_pg_world(self.pg_world.pbr_worldNP, self.pg_world.physics_world)

        self.scene_manager._traffic_manager.vehicles.append(self.breakdown_vehicle.vehicle_node.kinematic_model)
        self.alert = self.scene_manager._objects_manager.spawn_one_object(
        ...
        for p in pos:
            cone = self.scene_manager._objects_manager.spawn_one_object(
            ...
            self.scene_manager._traffic_manager.vehicles.append(cone)
        ...
        for v_long, v_t in zip(v_pos, v_type):
            v = self.scene_manager._traffic_manager.spawn_one_vehicle(v_t, lane, v_long, False)
        ...
        for p in pos:
            p_ = (p[0] + 5, -p[1])
            cone = self.scene_manager._objects_manager.spawn_one_object("Traffic Cone", lane, ("3R0_0_", "3R0_1_", 0), *p_)
            cone.attach_to_pg_world(self.pg_world.pbr_worldNP, self.pg_world.physics_world)
            self.scene_manager._traffic_manager.vehicles.append(cone)

        v_pos = [14, 19]
        for v_long in v_pos:
            v = self.scene_manager._traffic_manager.spawn_one_vehicle(
                self.scene_manager._traffic_manager.random_vehicle_type(), lane, v_long, False
            )
            v.attach_to_pg_world(self.pg_world.pbr_worldNP, self.pg_world.physics_world)

        alert = self.scene_manager._objects_manager.spawn_one_object(
            "Traffic Triangle", lane, ("3R0_0_", "3R0_1_", 0), -35, 0
        )
        alert.attach_to_pg_world(self.pg_world.pbr_worldNP, self.pg_world.physics_world)
        self.scene_manager._traffic_manager.vehicles.append(alert)
        ...
        for p in pos:
            p_ = (p[0] + 5, p[1] * 3.5 / 3)
            cone = self.scene_manager._objects_manager.spawn_one_object("Traffic Cone", lane, ("4C0_0_", "4C0_1_", 2), *p_)
            cone.attach_to_pg_world(self.pg_world.pbr_worldNP, self.pg_world.physics_world)
            self.scene_manager._traffic_manager.vehicles.append(cone)
        ...
        for p in pos:
            p_ = (p[0] + 60, -p[1] * 3.5 / 3)
            cone = self.scene_manager._objects_manager.spawn_one_object("Traffic Cone", lane, ("4C0_1_", "5R0_0_", 0), *p_)
            cone.attach_to_pg_world(self.pg_world.pbr_worldNP, self.pg_world.physics_world)
            self.scene_manager._traffic_manager.vehicles.append(cone)
