...
class TrafficCone(TrafficSign):
    ...
    def __init__(
        ...
        self.coordinate: NodePath = NodePath(self.body_node)
        self.coordinate.setPos(panda_position(self.position, self.HEIGHT / 2))
        self.dynamic_nodes.append(self.body_node)
        self.coordinate.setH(panda_heading(self.heading))
        if self.render:
            model = self.loader.loadModel(AssetLoader.file_path("models", "traffic_cone", "scene.gltf"))
            model.setScale(0.02)
            model.setPos(0, 0, -self.HEIGHT / 2)
            model.reparentTo(self.coordinate)
