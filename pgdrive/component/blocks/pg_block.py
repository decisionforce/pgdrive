...
class PGBlock(BaseBlock):
    ...
    """
    def __init__(self, block_index: int, pre_block_socket: PGBlockSocket, global_network: RoadNetwork, random_seed, ignore_intersection_checking=None):

        # TODO(pzh) remove None
        assert ignore_intersection_checking is not None

        self.name = str(block_index) + self.ID
        super(PGBlock, self).__init__(block_index, global_network, random_seed, ignore_intersection_checking=ignore_intersection_checking)
        # block information
