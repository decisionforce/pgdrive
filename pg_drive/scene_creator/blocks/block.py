...
class Block(Element):
    ...
    def __init__(self, block_index: int, pre_block_socket: BlockSocket, global_network: RoadNetwork, random_seed):
        ...
        if self.render:
            # render pre-load
            self.road_texture = self.loader.loadTexture(VisLoader.file_path(VisLoader.path, "textures","sci","color.jpg"))
            self.road_texture.setMinfilter(SamplerState.FT_linear_mipmap_linear)
            self.road_texture.setAnisotropicDegree(8)
            self.road_normal = self.loader.loadTexture(VisLoader.file_path(VisLoader.path, "textures","sci","normal.jpg"))
            self.ts_color = TextureStage("color")
            self.ts_normal = TextureStage("normal")
            self.side_texture = self.loader.loadTexture(VisLoader.file_path(VisLoader.path, "textures","side_walk","color.png"))
            self.side_texture.setMinfilter(SamplerState.FT_linear_mipmap_linear)
            self.side_texture.setAnisotropicDegree(8)
            self.side_normal = self.loader.loadTexture(VisLoader.file_path(VisLoader.path, "textures","side_walk","normal.png"))
